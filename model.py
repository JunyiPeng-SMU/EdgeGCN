...
class EdgeCNN_Net(torch.nn.Module):
    def __init__(self,channel,num_features,num_layer):
        ...
        self.conv01 = EdgeCNN(num_features, channel, num_layer,channel,dropout=0.5)
        self.lin1 = torch.nn.Linear(channel, channel //2)
        self.lin2 = torch.nn.Linear(channel //2, 2)
        self.act1 = torch.nn.ReLU()  # ELU
    def forward(self, data):
        x, edge_index = data.x, data.edge_index
        x = self.conv01(x=x,edge_index=edge_index)
        out = self.lin1(feature1)
        out = self.act1(out)
        out = self.dropout(out)
        out = torch.sigmoid(self.lin2(out)).squeeze(1)
        return out